...
class TestNginxExTemplate:
    ...
    def setup_method(self):
        self.oc_api = OpenShiftAPI(pod_name_prefix="nginex")
        json_raw_file = self.oc_api.get_raw_url_for_json(
            container="nginex", dir="imagestreams", filename="nginx-rhel.json"
        )
    ...
    def test_template_inside_cluster(self):
        template_json = self.oc_api.get_raw_url_for_json(
            container="nginex", dir="openshift/templates", filename="nginx.json"
        )
        ...
        assert self.oc_api.template_deployed(name_in_template="nginex")
    ...
    def test_template_by_request(self):
        ...
        assert self.oc_api.deploy_template(
            template=template_json, name_in_template="nginex", expected_output="Welcome to your static nginx application",
            openshift_args=["SOURCE_REPOSITORY_REF=master", f"NGINX_VERSION={VERSION}", "NAME=nginx-example"]
        )
        assert self.oc_api.template_deployed(name_in_template="nginex")
        assert self.oc_api.check_response_outside_cluster(
            name_in_template="nginx-example", expected_output="Welcome to your static nginex application"
        )
